...
<xsl:for-each select="ArrayOfWeaponSummary/WeaponSummary">
  ...
  <TR>
    ...
    <TD class="cellborder"><xsl:value-of select="ShotsHit"/> (<xsl:value-of select="format-number(number(PercentageShotsHit) * 100, '#0.00')"/>%)</TD>
  </TR>
  <TR>
  	<TD class="cellborder">Shots Missed</TD>
    <TD class="cellborder"><xsl:value-of select="ShotsMissed"/> (<xsl:value-of select="format-number(number(PercentageShotsMissed) * 100, '#0.00')"/>%)</TD>
  </TR>
  <TR>
...
<metaInformation>
<scenarios ><scenario default="yes" name="Main" userelativepaths="yes" externalpreview="no" url="ws1.xml" htmlbaseurl="" outputurl="" processortype="msxmldotnet" profilemode="0" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext=""/></scenarios><MapperInfo srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/>
</metaInformation>
-->